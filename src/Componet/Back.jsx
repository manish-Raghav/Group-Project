...
import { Backdt } from '../Data/Backdt';
...
const Back = () => {
  ...
  return (
    ...
    {
   Backdt.slice(page*10-10, page*10).map((el, index) => {
...
return (
...

  {
    ...
    Backdt.length>0 && <div className='flex gap-2 mt-10 text-lg'>

<span className='cursor-pointer bg-slate-300 w-14 mr-4 text-center hover:scale-95' onClick={()=>datashow(page-1)}>Pre</span>
...
return <span key={i}>{i + 1}</span>; 
    ...
    <span className='cursor-pointer bg-slate-300 w-14 ml-4 text-center hover:scale-95' onClick={()=>datashow(page+1)}>Next</span>
</div>
